    ...
    <x-auth.card>
        <x-slot name="header">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="auth-title text-2xl">Create an account</h1>
                    <p class="auth-subtitle mt-1">Join LenSymphony</p>
                </div>
                <a href="{{ route('login') }}" class="btn btn-outline text-sm">Log in</a>
            </div>
        </x-slot>
        ...
        <form action="{{ route('register') }}" method="POST" enctype="multipart/form-data" class="space-y-5" novalidate>
            ...
            <p class="text-xs text-center text-slate-500 -mt-4">PNG/JPEG, max 2MB, 512x512px</p>
            @error('avatar') <p class="form-error text-center">{{ $message }}</p> @enderror

            <!-- Username -->
            <div class="form-group">
                <label for="name" class="form-label">Username <span class="text-red-400">*</span></label>
            ...
            <!-- Confirmation -->
            <div class="form-group">
                <label for="password_confirmation" class="form-label">Confirm password <span class="text-red-400">*</span></label>
                <div class="input-icon-wrapper">
                    <svg class="input-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <p class="text-xs text-center text-slate-500 mt-4">
                By creating an account, you agree to LenSymphony's terms of service.
            </p>
        </form>
    </x-auth.card>
